  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-5">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Campañas y Promociones</h1>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-2">
            <label htmlFor="status-filter" className="text-xs font-medium text-gray-700 whitespace-nowrap">
              Filtrar por Estado:
            </label>
            <select
              id="status-filter"
              ...
              className="rounded-md border border-gray-300 bg-white py-1.5 px-3 text-sm focus:border-amber-500 focus:ring-amber-500"
            >
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="type-filter" className="text-xs font-medium text-gray-700 whitespace-nowrap">
              Tipo:
            </label>
            <select
              id="type-filter"
        ...
        {filteredCampaigns.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredCampaigns.map((camp) => {
              const typeInfo = TYPE_LABELS[camp.type] || { label: camp.type, color: 'bg-gray-100 text-gray-800' }
                ...
                >
                  <div className="space-y-3">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <h2 className="font-bold text-gray-900 text-base">{camp.name}</h2>
                        <p className="text-xs text-gray-500 line-clamp-2 mt-0.5">{camp.description}</p>
                      </div>
                      <span
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 grid grid-cols-4 gap-2 text-center text-xs">
                      <div>
                        <span className="block text-gray-400 font-medium">Enviados</span>
                        <span className="font-bold text-gray-800">{camp.stats?.sent || 0} enviados</span>
                      </div>
                      <div>
                        <span className="block text-gray-400 font-medium">Abiertos</span>
                        <span className="font-bold text-gray-800">{camp.stats?.opened || 0}</span>
                      </div>
                      <div>
                        <span className="block text-gray-400 font-medium">Conv.</span>
                        <span className="font-bold text-gray-800">{camp.stats?.converted || 0}</span>
                      </div>
                      <div>
                        <span className="block text-gray-400 font-medium">Ingresos</span>
                        <span className="font-bold text-emerald-600">${camp.stats?.revenue || 0}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 space-y-5">
            <div className="flex justify-between items-center border-b pb-3">
              <h3 className="text-lg font-bold text-gray-900">Crear Nueva Campaña</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-4">
              <div>
                <label htmlFor="camp-name" className="block text-xs font-medium text-gray-700 mb-1">
              ...
              <div>
                <label htmlFor="camp-desc" className="block text-xs font-medium text-gray-700 mb-1">
                  Descripción
                </label>
                <textarea
                  id="camp-desc"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="camp-type" className="block text-xs font-medium text-gray-700 mb-1">
                    Tipo de Campaña
                  </label>
                  <select
                    id="camp-type"
                ...
                <div>
                  <label htmlFor="camp-channel" className="block text-xs font-medium text-gray-700 mb-1">
                    Canal Principal
                  </label>
                  <select
                    id="camp-channel"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="camp-audience" className="block text-xs font-medium text-gray-700 mb-1">
                    Audiencia Objetivo
                  </label>
                  <select
                    id="camp-audience"
                ...
                <div>
                  <label htmlFor="camp-discount" className="block text-xs font-medium text-gray-700 mb-1">
                    Descuento (%)
                  </label>
                  <input
                    type="number"
              ...
              {type === 'REFERRAL' && (
                <div>
                  <label htmlFor="camp-referral" className="block text-xs font-medium text-gray-700 mb-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm font-mono focus:border-amber-500 focus:ring-amber-500"
                  />
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="camp-start" className="block text-xs font-medium text-gray-700 mb-1">
                    Fecha Inicio
                  </label>
                  <input
                    type="date"
                ...
                <div>
                  <label htmlFor="camp-end" className="block text-xs font-medium text-gray-700 mb-1">
                    Fecha Fin (Opcional)
                  </label>
                  <input
                    type="date"
